...
public class D37_JavaStreamApi {
	...
	private static List<String> getAllMovieTitlesWithRuntimeInMinutesLT90() {
		...
		movieTitlesWithRuntimeInMinutesLT90 = movies.stream().peek(System.out::println)
				.filter(m -> m.getRuntimeInMinutes() < 90).peek(System.out::println).map(Movie::getTitle)
	...
	private static List<String> getAllYearsAsSortedList() {
		...
		years = movies.stream().map(Movie::getYear) // .map(movie -> movie.getYear())
				.sorted().distinct().toList().reversed();
	...
	private static boolean isThereAHorrorMovieAfter1982() {
		...
		exists = movies.stream()
				.anyMatch(movie -> movie.getGenres().contains(Genre.HORROR) && movie.getYear().compareTo("1982") > 0);
	...
	private static Optional<Movie> getLongestMovie() {
		...
		longestMovie = movies.stream()
				.max((movie1, movie2) -> Integer.compare(movie1.getRuntimeInMinutes(), movie2.getRuntimeInMinutes()));
	...
	private static List<Movie> getTop5HorrorMovies() {
		...
		horrorMovies = movies.stream().filter(m -> m.getGenres().contains(Genre.HORROR))
				.sorted((movie1, movie2) -> Double.compare(movie2.getRating(), movie1.getRating())).limit(5).toList();
	...
	private static OptionalDouble getAverageRatingOfAllMovies() {
		...
		averageRating = movies.stream().mapToDouble(Movie::getRating) // .mapToDouble(m -> m.getRating())
				.average();
	...
	private static void printAllThrillersWithRatingGE7() {
		...
		movies.stream().filter(movie -> movie.getGenres().contains(Genre.THRILLER))
				.filter(movie -> movie.getRating() >= 7).map(movie -> movie.getTitle() + " (" + movie.getYear() + ")")
